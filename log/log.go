...
const (
	...
	LOG_LEVEL_DEV   = LOG_LEVEL_DEBUG | LogLevel(LOG_DEV)
	LOG_LEVEL_ALL   = LOG_LEVEL_DEV
)
...
func CrashLog(file string) {
	_, err := os.OpenFile(file, os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
...
func Dev(v ...interface{}) {
	_log.Dev(v...)
}

func Devf(format string, v ...interface{}) {
	_log.Devf(format, v...)
}
...
func GetLogFile(date string) string {
	filePath := _log.GetCurrentLogFile()
	if filePath == "" {
		return filePath
	}
	if date != "" {
...
func (l *logger) rotate() error {
	if (!l.dailyRolling) && (!l.hourRolling) {
		return nil
	}
...
func (l *logger) logRaw(record string) {
...
func (l *logger) Dev(v ...interface{}) {
	l.log(LOG_DEV, v...)
}

func (l *logger) Devf(format string, v ...interface{}) {
	l.logf(LOG_DEV, format, v...)
}
...
func Newlogger(w io.Writer, prefix string) *logger {
	return &logger{_log: log.New(w, prefix, LstdFlags), level: LOG_LEVEL_ALL, highlighting: false}
}

//Print for grom
func (l *logger) Print(values ...interface{}) {
	defer Reconver(l, values)

	sqlPart := strings.Split(values[3].(string), "?")

	for key, value := range values[4].([]interface{}) {
		...
		if _, ok := value.(string); ok {
			value = fmt.Sprintf("'%s'", strings.Trim(value.(string), "'"))
		}

		sqlPart[key] = fmt.Sprintf("%s %+v", sqlPart[key], value)
	}

	l.logRaw(fmt.Sprintf("%s: [%s] {%s} %s", values[1], values[0], values[2], strings.Join(sqlPart, "")))

}

//Printf for zk
func (l *logger) Printf(info string, values ...interface{}) {
	l.Infof(info, values...)
}

func (l *logger) DebugPrintf(info string, values ...interface{}) {
	l.Debugf(info, values...)
}

func Reconver(l *logger, values ...interface{}) {
	if e := recover(); e != nil {
		l.logRaw(fmt.Sprintf("%+v", values))
	}
}
